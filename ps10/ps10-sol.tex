...
\begin{proof}
Assume to the contrary that $u$ is a key vertex of $G$, the deletion of $u$ disconnects $G$. Then it also disconnect $T$, but $T - u$ is a tree. Therefore $u$ is not a key vertex of $G$.
\end{proof}
\begin{algorithmic}[1]
\State $u = V[1]$
...
\end{algorithmic}
\subproblem %b
$T - u$ is a spanning tree of $G - u$. Let the deletion order be the post traversal order of $T$. In each deletion, a leaf node of $T$ aka non-key vertex of $G$ is deleted and the remaining tree is a spanning tree of the remaining graph. By induction, the post traversal order of DFS tree is a valid deletion order.\\[1em]
\ta{DFS1}$(G,u,result)$
\begin{algorithmic}[1]
\State $u.vis = \it True$
\For{each $v$ in $G.Adj[u]$}
...
\end{tikzpicture}
\problem
Note: it is required to add a precondition that $|V|\le |E|$ \cite{23-1}, to ensure the existence of second-best minimum spanning tree (SMST).
\subproblem %a
...
\end{tabular} \\[1em]
The running time of $\ta{Prim}$, in the textbook, is related to the chosen of priority queue. If we implement it with brute force, where $\ta{ExtractMin}$ takes O($|V|$) and $\ta{DecreaseKey}$ takes $O(1)$. Then $T(Prim) = O(|V|^2+E) = O(|V|^2)$.
...
For each iteration of outer loop from line 4 to line 16, assume $i$ increases by $\Delta_i$. And assume in the inner loop from line 6 to line 9, $i$ increases by $\delta_i$.
\begin{itemize}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        If $(L_t, R_t)\notin C_{opt}$, choose one interval $I$ in $C_{opt}\cap S$. $C\cup (L_t, R_t)$ covers $C\cup I$. So $C\subseteq C_{opt}-I\cup (L_t, R_t)$ is a subset of some smallest cover of $X$. The invarient holds for the next iteration $i=i_{10}, j=t$.
    \end{enumerate}
\end{proof}
...
\begin{enumerate}
    ...
    \item Correct. \label{stgy:2} \textit{Proof.} Map each activity $(l, r)$ to $(-r, -l)$. The optimal selection on mapped intervals is an optimal selection. Since strategy ``choose activity ends first'' is correct. The reflection strategy ``choose activity starts last'' is correct. \hfill\qedsymbol
    ...
    {\it Opt} = \{(1,2),(2,4),(4,6),(6,7)\}. But the strategy firstly choose $(3,5)$ which has 2 conflicts, and gives a size 3 selection.
    \item Correct. For any optimal selection $S$, if any $x\in S$ completely contains another activity $y$, $S - \{x\} \cup \{y\}$ is an optimal selection. After discarding useless activities, an activity ends last must starts last (if it starts earlier than $y$, then it contains $y$). The strategy is correct by the correctness of strategy \ref{stgy:2}.
\end{enumerate}
\subproblem %b
