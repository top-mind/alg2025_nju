...
Together, (1) and (2) implies that
$$f(n)\ge f(\lfloor n/2\rfloor)+1+n\text{ mod } 2+k$$
\begin{equation}
f(n)\ge f(\lfloor n/2\rfloor)+1+n\text{ mod } 2
\end{equation}
But $n$ can be obtained by double $\lfloor n/2\rfloor$, and increase if $n$ is odd.
\begin{equation}
...
\problem %2
\subproblem %a
Assume to the contrary that there is no codeword of length 1. Then the root of the Huffman tree cannot have a leaf as direct child. Suppose one child of root has children $A, B$, and the other has children $C, D$. Without lose of generality, assume $w(A)\le w(B), w(C)\le w(D)$, and the merge of $A$ and $B$ happened first. We have $w(A)\le w(B)\le w(C)\le w(D)\le w(A)+w(B)$. Some character occurs more than $2/5$, so $w(D)>2/5$. Then $w(C)\ge w(B)\ge(w(A)+w(B))/2>1/5$. $1=w(A)+w(B)+w(C)+w(D)>2/5+1/5+2/5=1$, contradictory. Therefore, there is a codeword of length 1.
\subproblem %b
Assume to the contrary that there is a codeword $A$ of length 1. Since all characters occurs less than 1/3, there are at least 4 characters. Suppose $A$ is merged with $X$, and $X$ is merged by $B$ and $C$, $w(B)\le w(C)$. Then we have $w(B)\le w(C)\le w(A)<1/3$. $1=w(A)+w(B)+w(C)>1/3\times3=1$, contradictory. Therefore, there is no codeword of length 1.
\section*{Bonus Problem}\setcounter{subsection}{0}
\subproblem %a
Let $w_i=\frac{|S_i|}{c(S_i)}$. Choose the set $X$ with largest $w$, remove elements in $X$ from other $S_i$, and recurse.\\[1em]
\begin{algorithmic}[1]
\For{$i=1$ to $k$}
    \State $w[i]=|S_i|/c(S_i)$
\EndFor
...
\State \Return $A$
\end{algorithmic}
\paragraph{Running time} the number of iterations of loop on lines 3-9 bounds by $\min\{n,k\}$.
...
\end{proof}
Remark: it is necessary to assume positive weight. If we assume non-negative weight, the following graph has completely disjoint MST (green) and shortest-path tree (red).
\begin{center}
\begin{tikzpicture}[graph]
    \node (s) at (0,0) {$s$};
    \node (x) at (90:1.2) {$x$};
...
\end{equation*}
Since $-\lg(r)\ge 0$, the problem is transformed to non-negative weight shortest path problem, which can be solved by Dijkstra's algorithm. But we have to deal with 2 issues.
\begin{description}
\item[Round-off error] It is out of scope of our algorithm course \cite{ALG:intro}. 但是我一时兴起了, and we will briefly show the error is acceptable using IEEE 754 format (base 2). Suppose normal floating point number $0<r<1$ is represented as $r=c\times 2^{-q}$, $1 \le c < 2$ is a $p$ bit precision, and $0 < q < 2^e$ is an $e$ bit exponent.\\
...
For an $1+e+p$ bit float number $r$, $\lg(r)$ needs $e+p$ bit precision. So if we use \verb|float|(1+8+23) for $r$ and \verb|double|(1+11+52) for $-\lg r$, the error will be acceptable.
\item[{\boldmath $r=0$}] Let $-\lg(0)=\infty$, then Dijkstra's algorithm on $(G,w,s)$ will ignore the $\infty$ weight edges since $v.d > u.d + \infty$ never holds.
...
So the running time is $O(T(\ta{Dijkstra})+T(\ta{Path})+T(\ta{BFS}))=O(V\lg V+E)$ (by implementing the min-priority queue with a Fibonacci heap). No matter how min-priority queue is implemented, the bottleneck is \ta{Dijkstra}.
\problem %5
...
\paragraph{Correctness (schema)} Map $(u, v)$ to integers between $1$ and $|E|$. The rule is, if $w(e_1) < w(e_2)$, then $f(e_1) < f(e_2)$. Let new weight function $w'(u, v)=1/2^{f(u,v)}$. Then the shortest path  on $G$ with $w'$ has largest minimum height. This shows the connection between max-height problem and shortest-path problem. The modified Dijkstra's algorithm utilize this idea.
\problem %6
Modify Bellman-Ford algorithm. Change \ta{Initialize-Single-Source} to \ta{Initialize-Multiple-Source} that sets each $v.d$ to $0$.\\[1em]
...
\bibitem{ALG:intro} \url{https://chaodong.me/teaching/alg/2025/#h1.1.1_description}
``This course introduces to you some of the most basic and important algorithms that lie at the core of computer science, along with the various techniques used in the design and analysis of these algorithms.'' 
\end{thebibliography}
\end{document}