...
\end{proof}
Remark: It is necessary to assume positive weights. If we assume non-negative weights, the following graph has a completely disjoint MST (green) and shortest-path tree (red, rooted at $s$).
\begin{center}
\begin{tikzpicture}[graph]
    \node (s) at (0,0) {$s$};
    \node (x) at (90:1.2) {$x$};
...
\end{equation*}
Since $-\lg(r)\ge 0$, the problem is transformed to a non-negative weight shortest path problem, which can be solved by Dijkstra's algorithm. (In the real world, instead of computing $\lg$\footnote{Float point error might be a problem. We can estimate the error under IEEE 754 format (base 2). Suppose normal floating point number $0<r<1$ is represented as $r=c\times 2^{-q}$, $1 \le c < 2$ is a $p$ bit precision, and $0 < q < 2^e$ is an $e$ bit exponent.\\
...
For an $1+e+p$ bit float number $r$, $\lg(r)$ needs $e+p$ bit precision. So if we use \texttt{float}(1+8+23) for $r$ and \texttt{double}(1+11+52) for $-\lg r$, the error will be acceptable.}, we can modify the \ta{Initialize-Single-Source}, \ta{Relax} procedure and change the min-priority queue to a max-priority queue. See the next problem 5.)\\

Wait, the trick has a gap when {\boldmath $r=0$}. We can let $-\lg(0)=\infty$, then Dijkstra's algorithm on $(G,(-\lg)\circ r,s)$ will ignore the $\infty$ weight edges since $v.d > u.d + \infty$ never holds.
If $t.\pi\neq\it NIL$, we are done.
If $t.\pi=\it NIL$, there is no path from $s$ to $t$ that does not traverse an edge with reliability 0. Run \ta{BFS}$(G,s)$ to find a path from $s$ to $t$. Editor's comment: gild the lily.\\
...
\end{proof}
\paragraph{Running time} Same as \ta{Bellman-Ford}, it takes $O(VE)$.
